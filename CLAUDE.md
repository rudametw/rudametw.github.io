...
## Publications (`data/publications.yaml`, since 2026-09-12)

Rendered by `layouts/publications/section.html` from `data/publications.yaml`,
grouped by `type` then year, with the author's `rank` / `note` pills and
HAL / DOI / PDF / extra links. `content/publications/_index.md` is front matter
only (generated by `move-content.sh`). The 2015 hand-written table is retired;
its eleven non-HAL items (2011 IGI chapter, two 2008 papers, the 2012 SBES
workshop paper, three demos, three industry talks, the 2007 master's thesis)
live in the YAML as `manual-…` entries, words as they were.

Pipeline, run by the author, never by CI:

```bash
./scripts/fetch-hal-bibtex.sh    # HAL API -> publications.bib (rows=5000), committed
...
sample, external. The nine local `/docs/` files from the old page are attached
